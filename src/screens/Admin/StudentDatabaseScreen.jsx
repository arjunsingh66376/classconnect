...
import {
  View, Text, StyleSheet, ScrollView, ActivityIndicator, Alert, Dimensions, TouchableOpacity, TextInput,
} from 'react-native';
import { Table, Row } from 'react-native-table-component';
import Orientation from 'react-native-orientation-locker';

const BACKEND_URL = 'http://192.168.1.211:5000'; // Your backend endpoint
const { width } = Dimensions.get('window');

export default function StudentDatabaseScreen() {
  const [tableHead] = useState([
    'Class', 'Roll No', 'Name','Attendance','Present','Absent', 'Subjects', 'Percentage', 'Class Teacher', 'Total Fees','Fees Paid', 'Fees Due',
  ]);
  ...
  // Handler for changing new row input
  const onChangeNewRow = (field, value) => {
    setNewRow(prev => ({ ...prev, [field]: value }));
  };

  // Save new student to backend
  const onSaveNewRow = async () => {
    ...
    try {
      // Transform subjects string to expected backend subject structure if needed
      // Example: expect comma separated "Math:90/100,English:85/100"
      ...
      // Success: refresh data, clear newRow and hide add row
      await fetchStudents();
      setAddingRow(false);
      setNewRow({
        studentClass: '',
  ...
  // Render editable new row as a View with inputs
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator style={{ flexGrow: 0 }}>
          <View>
            ...
            <ScrollView style={{ maxHeight: 500 }}>
              ...
              {/* Render editable new row as a View and NOT inside Table/Row */}
              {addingRow && renderEditableRow()}
            </ScrollView>
          </View>
        </ScrollView>
      }
